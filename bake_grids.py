#!/usr/bin/env python3
"""bake_grids.py — writes the current feed.json order into the six baked film grids in index.html
...
def card(v):
    ...
    return (f'      <div class="film-card" data-cursor data-yt="{i}">\n'
            f'        <img class="film-thumb" src="https://i.ytimg.com/vi/{i}/hq720.jpg" alt="{t}" loading="lazy" '
            f'onerror="this.onerror=null;this.src=\'https://img.youtube.com/vi/{i}/hqdefault.jpg\'" />\n'
...
for key, vids in feed.items():
    ...
    body = "".join(card(v) for v in vids[:COUNT.get(key, 6)])
